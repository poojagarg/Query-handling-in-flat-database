	...
	{	
		...
		for(index_row=0;index_row<NScatter;index_row++){
			...
			for(i=0;i<cn;i++)
			{
				/*if(i!=0)
				{
					int prev=(eval(ProcessTableBuffer[index+condindex[i-1]],condval[i-1],operator[i-1]));
					int curr=(eval(ProcessTableBuffer[index+condindex[i]],condval[i],operator[i]));
					printf("\n %d %d******************^^^^^^^^^^^^ %d",prev,curr,ProcessTableBuffer[index]);
					if(!(evalRO(prev,curr,ro[i-1])))
					{	condmatch=0;
						printf("\nzzzzzzzzzzzzzzzzzzzzzzz %d %d %d %d %d %d %d %d",ProcessTableBuffer[index+condindex[i-1]],condval[i-1],ro[i-1],prev,curr,i-1,ro[0],(evalRO(prev,curr,ro[i-1])));
						printf("\nzzzzzzzzzzzzzzzzzzzzzzz %d %d",ProcessTableBuffer[index+condindex[i]],condval[i]);
						break;
				...
				printf("\n********zzzz***%d *** %d *** %d  %d %d ",ProcessTableBuffer[index+condindex[i]],condval[i],condmatch,i,cn);
